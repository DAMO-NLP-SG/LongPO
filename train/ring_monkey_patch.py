...
def blockwise_cross_entropy(inputs, targets, block_size=16):
    """
    Calculate cross-entropy loss in blocks.
    
    Parameters:
    - inputs: Tensor of predictions (logits), shape (batch_size, num_classes)
    - targets: Tensor of true labels, shape (batch_size,)
    - block_size: int, Size of each block
    
    Returns:
    - loss: Scalar tensor that represents the mean loss across all blocks
    """
    ...
    for block_idx in range(num_blocks):
        # Compute start and end indices for the current block
        start_idx = block_idx * block_size
        end_idx = min((block_idx + 1) * block_size, batch_size)

        # Select the block
        input_block = inputs[start_idx:end_idx]
        target_block = targets[start_idx:end_idx]
        
        # Compute the cross-entropy loss for the current block
        block_loss = F.cross_entropy(input_block, target_block, reduction='none')
        
        # Accumulate the block loss
        loss += block_loss.sum()
        
    # Take the average by dividing the total loss by the batch size
