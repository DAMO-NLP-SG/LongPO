...
class LongDPOHybridTrainer(LongDPOTrainer):
...
class LongDPOFullMTFixedJointUlyssesTrainer(LongDPOFullJointUlyssesTrainer):
    ...
    @staticmethod
    def sum_sub_logps_per_chunk(logps, loss_mask, start, end, num_chunks=4):
        """
        Calculate the sum of logps for each chunk (specified by the loss_mask) for a given subsequence.
        
        Args:
            logps (torch.Tensor): Tensor of shape (batch_size, sub_seq_length), log probabilities for the subsequence
            loss_mask (torch.Tensor): Tensor of shape (batch_size, seq_length), indicating the chunks
            start (int): The starting index of the subsequence in the original sequence
            end (int): The ending index of the subsequence in the original sequence
            num_chunks (int): Number of chunks to sum over, default is 4
        Returns:
            torch.Tensor: Tensor with shape (batch_size, num_chunks) containing the sums of logps per chunk.
        """
        batch_size, sub_seq_length = logps.shape
        seq_length = loss_mask.shape[1]
        
        # Initialize the result tensor
        chunk_sums = torch.zeros((batch_size, num_chunks), device=logps.device)
        # chunk_sums.requires_grad = True
        for b in range(batch_size):
            ...
            # Calculate the sub_seq_range within the original chunks
            sub_seq_indices = torch.arange(start, end, device=logps.device)
            sub_seq_logps = logps[b]
            ...
            for chunk in chunk_indices:
                ...
                relevant_chunk_positions = [idx for idx in chunk if start <= idx < end]
                if len(relevant_chunk_positions) > 0:
                    chunk_position_indices = torch.tensor(relevant_chunk_positions, device=logps.device) - start
                    chunk_logps = sub_seq_logps[chunk_position_indices]
                    
                    chunk_sum = torch.sum(chunk_logps)
                    chunk_sums[b, current_chunk_idx] += chunk_sum
                    
                    sub_seq_position += len(relevant_chunk_positions)
    ...
    def mt_concatenated_forward(
        ...
        # shift_labels = labels[..., 1:].contiguous()
        # if local_all_logits.shape[1] != local_labels.shape[-1]:
        #     shift_logits = local_all_logits[:, :-1, :].contiguous()
        # else:
        #     shift_logits = local_all_logits
        
        # local_logps = self.get_batch_logps_v2(
        #     local_all_logits,
        #     local_labels,
        #     average_log_prob=False,
        #     is_encoder_decoder=self.is_encoder_decoder,
        #     label_pad_token_id=self.label_pad_token_id,
        # )
        local_ce_loss = self.get_ce_loss(local_all_logits, local_labels)
        batch_size, seq_length = local_labels.shape
        # print(seq_length, local_ce_loss.shape, self.accelerator.process_index)
        local_logps = -local_ce_loss.view(batch_size, seq_length-1)
        local_loss_mask = local_labels[:, 1:] != self.label_pad_token_id
        # tmp_logps = (local_logps * local_loss_mask).sum(dim=-1)
        
        global_loss_mask = concatenated_batch["concatenated_labels"][:, 1:] != self.label_pad_token_id
        
        
        # def get_mask_index(logps, mask):
        #     # Step 1: Identify the start points of each chunk
        #     chunk_starts = torch.diff(mask, dim=1, prepend=torch.zeros((logps.shape[0], 1), device=mask.device)) == 1
        #     # Ensure that first elements which are 1 are also marked as starts
        #     chunk_starts[:, 0] = mask[:, 0] == 1
        #     # Convert chunk_starts to float to use it in cumulative sum
        #     chunk_starts = chunk_starts.float()
        #     # Step 2: Create cumulative sum to identify chunk indices
        #     chunk_indices = torch.cumsum(chunk_starts, dim=1)
        #     # Ensure chunk_starts are correct after cumulation by masking `mask`
        #     chunk_indices *= mask
        #     # Step 3: Adjust the indices to be sequential within chunks
        #     # Subtract 1 from non-zero cumulative sum to get correct 0-based indexing
        #     chunk_indices[mask == 1] -= 1
        #     result = torch.zeros(logps.shape[0], 4, device=logps.device)
        #     # # unique_indices.view(b, s) converts unique_indices to (b, s) shape
        #     # # tensor.gather is used to sum all the values that fall under same chunk id
        #     # print(unique_indices.view(b, s))
        #     # unique_indices = get_mask_index(mask)
        #     logps = logps * mask
        #     result.scatter_add_(1, chunk_indices.long(), logps)
        #     return result
        # # if self.accelerator.process_index == 0:
        # if self.accelerator.process_index != self.accelerator.num_processes - 1:
        #     gather_list = [torch.zeros_like(local_logps) if rank != self.accelerator.num_processes - 1 else torch.zeros(batch_size, seq_length-2, device=local_logps.device) for rank in range(self.accelerator.num_processes)]
        # else:
        #     gather_list = [torch.zeros_like(local_logps) if rank == self.accelerator.num_processes - 1 else torch.zeros(batch_size, seq_length, device=local_logps.device) for rank in range(self.accelerator.num_processes)]
        # # torch.distributed.gather(local_logps, gather_list, dst=0)
        # dist.all_gather(gather_list, local_logps)
        # assert gather_list[self.accelerator.process_index].sum() == local_logps.sum()
        # full_logps = torch.cat(gather_list, dim=1)
        # print(full_logps.size())
        # all_logps = get_mask_index(full_logps, global_loss_mask)
        
        def get_mask_chunk_index(logps, mask, start_index, end_index):
            # Step 1: Identify the start points of each chunk
            chunk_starts = torch.diff(mask, dim=1, prepend=torch.zeros((logps.shape[0], 1), device=mask.device)) == 1
            # Ensure that first elements which are 1 are also marked as starts
            chunk_starts[:, 0] = mask[:, 0] == 1
            ...
            # Subtract 1 from non-zero cumulative sum to get correct 0-based indexing
            chunk_indices[mask == 1] -= 1
            chunk_indices = chunk_indices[:, start_index:end_index]
            # print(chunk_indices)
            result = torch.zeros(logps.shape[0], 4, device=logps.device)
            # # unique_indices.view(b, s) converts unique_indices to (b, s) shape
            # # tensor.gather is used to sum all the values that fall under same chunk id
            # print(unique_indices.view(b, s))
            # unique_indices = get_mask_index(mask)
            logps = logps * mask[:, start_index:end_index]
            result.scatter_add_(1, chunk_indices.long(), logps)
            return result
        
        sub_seq_length = concatenated_batch["concatenated_labels"].shape[1] // self.accelerator.num_processes

        sub_seq_start = self.accelerator.process_index * sub_seq_length
        sub_seq_end = (self.accelerator.process_index + 1) * sub_seq_length
        if sub_seq_end - sub_seq_start == seq_length:
            sub_seq_end -= 1
        assert sub_seq_end - sub_seq_start == seq_length - 1

        assert global_loss_mask[:,sub_seq_start:sub_seq_end].sum() == local_loss_mask.sum()
        
        all_logps = get_mask_chunk_index(local_logps, global_loss_mask, sub_seq_start, sub_seq_end)
        # all_logps = self.sum_sub_logps_per_chunk(local_logps, global_loss_mask, sub_seq_start, sub_seq_end, num_chunks=4)

        torch.distributed.all_reduce(all_logps, op=torch.distributed.ReduceOp.SUM)
        # torch.distributed.all_reduce(tmp_logps, op=torch.distributed.ReduceOp.SUM)
        # a = tmp_logps.sum()
        # b = all_logps.sum()
        # if self.accelerator.process_index == 0:
        #     print("The gap:", a-b)
        # assert tmp_logps.sum() == all_logps.sum()
        # all_logps.requires_grad = True
        # all_logps = all_logps.sum
        # dist.broadcast(all_logps, src=0)
        # all_logps = self.sum_sub_logps_per_chunk(local_logps, global_loss_mask, sub_seq_start, sub_seq_end, num_chunks=4)
        # # all_logps.requi
        # torch.distributed.all_reduce(all_logps, op=torch.distributed.ReduceOp.SUM)

        # print("Gradient: ", all_logps.requires_grad, local_logps.requires_grad, all_logps.size())
        chosen_logps = all_logps[:len_chosen, :]
        rejected_logps = all_logps[len_chosen:, :]

        chosen_logits = local_all_logits[:len_chosen]
        rejected_logits = local_all_logits[len_chosen:]
    ...
    def get_batch_loss_metrics(
        ...
        (
            ...
            policy_chosen_logits,
            policy_rejected_logits,
            policy_chosen_lm_loss
        ) = self.mt_concatenated_forward(model, batch, ref_mode=False)
        for k in batch:
            # if k.startswith("chosen") and isinstance(batch[k], torch.Tensor):
        ...
        dist.all_reduce(reference_rejected_logps, op=torch.distributed.ReduceOp.SUM)
        dist.barrier()
        # torch.distributed.all_reduce(policy_chosen_loss, op=torch.distributed.ReduceOp.SUM)
        ...
        reference_chosen_logps = reference_chosen_logps.view(-1, 4)
        reference_rejected_logps = reference_rejected_logps.view(-1, 4)
        ...
        # print("chosen loss: ", policy_chosen_loss.mean())
        # print("gather sucessfully, begin computing loss.")
        losses, chosen_rewards, rejected_rewards = self.mt_dpo_loss(
            policy_chosen_logps,
            policy_rejected_logps,
            reference_chosen_logps,
            reference_rejected_logps,
        ...
        #     self.accelerator.device
        # )
        policy_chosen_loss = policy_chosen_lm_loss
...
class LongDPOFullMTJointUlyssesTrainer(LongDPOFullJointUlyssesTrainer):
    ...
    def long_concatenated_forward(
        ...
        # all_logps = (all_logps * loss_mask).sum(dim=-1)
        ...
        if ref_mode:
            return (chosen_logps, rejected_logps, chosen_logits, rejected_logits)
        local_lm_labels = self.extract_local_label(
            concatenated_batch["concatenated_lm_labels"],
            self.accelerator.process_index,
            self.accelerator.num_processes, 
            self.accelerator.device
        )[:len_chosen]
...
class LongSFTKLJointUlyssesTrainer(LongDPOFullJointUlyssesTrainer):
    ...
    def long_chosen_forward(
        ...
        local_probs = F.log_softmax(local_logits, dim=-1).mean(dim=-1)
        ...
        return chosen_loss, local_probs, kl_mask
    ...
    def get_batch_loss_metrics(
        ...
        policy_sft_loss, policy_probs, policy_probs_mask = self.long_chosen_forward(model, batch, ref_mode=False)
        ...
        for k in batch:
            ...
            if k.startswith(f"ref_") and isinstance(batch[k], torch.Tensor):
                ...
                batch[k] = batch[k].squeeze(dim=0)
        # print("Ref shape: ", batch["ref_chosen_input_ids"].shape)
        ref_probs, ref_probs_mask = self.long_chosen_forward(model, batch, ref_mode=True)
        mask_sum = policy_probs_mask.sum(dim=-1)
        value_probs = (policy_probs  * policy_probs_mask).sum(dim=-1)
        dist.all_reduce(value_probs, op=torch.distributed.ReduceOp.SUM)
        dist.all_reduce(mask_sum, op=torch.distributed.ReduceOp.SUM)

        ref_value_probs = (ref_probs * ref_probs_mask).view(-1, pair_num, ref_probs.shape[1]).sum(dim=1)
        dist.all_reduce(ref_value_probs, op=torch.distributed.ReduceOp.SUM)
        dist.barrier()
        kl_penalty = (value_probs - ref_value_probs).abs() / mask_sum
        ...
        metrics = {}
        metrics["sft_loss"] = policy_sft_loss.mean().item()
        metrics["kl_penalty"] = kl_penalty.mean().item()
        return loss.mean(), metrics